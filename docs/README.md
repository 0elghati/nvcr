...
backend. The static registry, provider API, and artifact resolver prove the
extension contracts, but production DCVC-RT still needs to be routed through
that provider API. No second production codec or provider exists.
...
The latest machine-readable performance matrix is kept under `../evidence/`. It
is diagnostic until a clean target run generated by
`scripts/benchmark_softwarex_matrix.py` replaces it. Old run dumps and working
notes are intentionally not part of the active documentation set.
