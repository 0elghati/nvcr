...
run_case() {
    ...
    local input="$5"
    local gop="$6"
    local output="$output_dir/nvcr_${label}_qp${qp}_gop${gop}.nvcr"
    local log
    local summary
    local payload
    local seconds
    ...
    echo "== $label qp=$qp gop=$gop profile=$profile =="
    log="$("$nvcr_bin" encode \
    ...
    if [[ -n "$jsonl" ]]; then
        ...
        printf '{"schema":"nvcr.benchmark.pair.v1","nvcr_commit":"%s","nvcr_dirty":%s,"resolution":"%s","input":"%s","size":"%s","fps":%s,"frames":%s,"qp":%s,"gop_size":%s,"engine_profile":"%s","payload_bytes":%s,"codec_time_seconds":%s,"throughput_fps":%s,"output":"%s"}\n' \
            "$commit" "$dirty" "$label" "$input" "$size" "$fps" "$frames_done" "$qp" "$gop" \
